...
const BlogSideBar = ({
  ...
  singleBlogTags,
  onCategorySelect,
  onTagSelect, searchTerm, handleSearch
}) => {
  // console.log(singleBlogTags)
  return (
    <div>
      <div className="">
        {/* search */}
            ...
            <div className="flex flex-wrap gap-y-3 gap-x-2">
              ...
              {categories?.map((category) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 ${selectedCategory?.id === category.id
                    ? "bg-[#FF693B] text-white"
                    : "bg-gray-200 text-gray-800 hover:bg-gray-300"
                    }`}
                >
        ...
        <div className='space-y-8'>
          ...
          <BlogTags tags={tags} selectedTag={selectedTag} onTagSelect={onTagSelect} singleBlogTags={singleBlogTags} />
        </div>
      </div>
    </div>
  );
