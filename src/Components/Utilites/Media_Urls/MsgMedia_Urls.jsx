...
const MsgMedia_Urls = ({ attachment }) => {
  const [downloading, setDownloading] = useState({});
  // Parse the attachment string to convert it into a JavaScript array
  let attachmentArray = [];
  try {
    attachmentArray = JSON.parse(attachment);
  } catch (error) {
    console.error("Error parsing attachment JSON:", error);
    // Handle parsing error gracefully if needed
  }

  // Check if attachmentArray is an array
  if (!Array.isArray(attachmentArray)) {
    // If attachmentArray is not an array or empty, handle accordingly
