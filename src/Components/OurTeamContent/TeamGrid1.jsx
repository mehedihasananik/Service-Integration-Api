...
export default function TeamGrid1() {
  const TeamCard = ({ name, title, image, roleLink }) => {
    const [isHovered, setIsHovered] = useState(false);

    return (
      <div
        className="bg-white rounded-xl border overflow-hidden flex flex-col h-full relative"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <div className="flex-grow flex items-center justify-center px-4 pt-3 relative">
          <img
            className={`w-full h-auto object-contain max-h-[400px] rounded-xl transition-transform duration-300 ${
              isHovered ? "" : ""
            }`}
            src={image}
            alt={name}
